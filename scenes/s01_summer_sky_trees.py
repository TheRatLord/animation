...
BANDS = [_h('#1d2b44'), _h('#27434f'), _h('#35604c'), _h('#5b8744'), _h('#9dbd5c')]
HOT = _h('#f6eaa8')
TRUNK = _h('#2a2e3c')
...
def tree(cv, x, y, r, s, rng, hk, hz, lean=0.0):
    ...
    r = r * 0.88
    y = y + 0.12 * r
    L = np.array([-0.3, -0.85, 0.3], np.float32)
    L /= np.linalg.norm(L)
    ...
    for ci, (cx, cy, rr) in enumerate(clumps):
        ...
        val = lam * 0.8 - 0.02 - 0.45 * low + rng.normal(0, 0.1, m)
        order = np.argsort(val)
        for i in order:
            px = cx + dx[i] * rr
            py = cy + dy[i] * rr
            dr = rr * rng.uniform(0.055, 0.11) * (1.0 if rad[i] < 0.85 else 0.75)
    ...
    # warm backlit rim: only the top / sun-side silhouette of the canopy catches the light
    k = max(2, int(0.05 * r * ss))
    above = np.zeros_like(al)
    above[k:] = al[:-k]
    left = np.zeros_like(al)
    kx = max(1, k // 2)
    left[:, kx:] = al[:, :-kx]
    rim = np.clip(al * (1 - np.minimum(above, 1)) * 1.0 + al * (1 - np.minimum(left, 1)) * 0.25, 0, 1)
    rim = cv2.GaussianBlur(rim, (0, 0), 0.6 * ss)
    rgb += (hot - rgb) * (0.6 * rim)[..., None]
    w, h = bx1 - bx0, by1 - by0
