...
FIRM, SOFT, SC, LS, TW = 0.55, 0.5, 0.55, 0.3, 0.12
...
def _tower(P, X, Y, H, seed, form=0.4, sun_z=0.5):
    ...
    common = dict(group=g, form=form, sun_z=sun_z, wrap=0.0, term_w=TW, rim_interior=0.0, side_scale=0.85,
                  term_noise=0.18, inner=0, sky=0.08, halo=0.0, rim_px=2.6, hot=1.0, lit_step=LS,
                  scallop=SC, levels=LEVELS, clump=0.22, firm=FIRM, soften=SOFT, rim_shadow=0.08,
    ...
    masses = [
        ...
        (dict(cx=cx + 0.3 * Hc, base_y=base - 0.14 * Hc, width=0.4 * Hc, height=0.42 * Hc, power=2.2,
              lump=0.14, lean=0.05, base_round=0.3), dict(size=0.28 * Hc, cast=0.0, light=1.0)),
...
def paint(pw, ph, W, H, ox, sun, seed=5, sky=None, form=0.7, sun_z=0.35, sun_y=-0.4, rim_y=0.33):
    ...
    # hot-gold lining on the sun-facing silhouette only (no halo glow): the lining looks at the real
    # afterglow (low, left), the painted masses at a higher key light (tops lit, bases in earth shadow)
    P.sun = np.array((float(sun[0]), Y(rim_y)), np.float32)
