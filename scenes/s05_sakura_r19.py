"""s05_sakura round 19: Painter19 = round-18 clustered five-petal dab crowns with a real value structure.

Reviewer (round 18 FAIL): 'sun-side rim glow missing', 'canopy value structure too even - lower half one uniform
magenta band; 3 clear tiers (pale lit tips / mid pink / violet shadow) with hard inner clump separations, lost
bottom edges; dabs a uniform scatter of equal squarish blobs - vary clump size, less rectangular dabs'.

  * palette: blue-violet core, crimson-mauve shade, rose body, light pink, pale tip, hot cream (warmer, less magenta);
  * every flower head snaps to one of THREE value tiers (TIERS) so neighbouring clumps separate with hard edges;
    the per-mass field is steeper (top -> underside), the lower half of the whole crown sinks into the violet
    core, clusters in the crown's own sun-shadow (Dsun) darken, sun-exposed clusters brighten;
  * sun-exposed clump tips (ex) push their sun-side blossoms to near-white cream;
  * dab size varies per cluster (big in the body of a mass, fine at the crown rim), rounder 5-petal dabs
    (shallower notches, softer AA) -> no squarish scatter;
  * a 2-4 px near-white warm-cream rim on every sun-facing blossom silhouette + stronger warm halation.
"""
import math
...
BANDS18 = np.array([
    (0.4, 0.28, 0.55),     # 0 blue-violet core
    (0.74, 0.37, 0.54),    # 1 crimson-mauve shade
    (0.94, 0.56, 0.67),    # 2 rose body
    (1.0, 0.75, 0.8),      # 3 light pink
    (1.04, 0.9, 0.9),      # 4 pale tip
    (1.1, 1.02, 0.95),     # 5 hot cream
], np.float32)
BANDS18_FAR = (BANDS18 * 0.72 + np.array([0.9, 0.86, 0.95], np.float32) * 0.28).astype(np.float32)
TIERS = (0.7, 2.3, 3.9)     # round 19: three clear value tiers (violet shadow / mid pink / pale lit tips)


def _tier(v):
    return TIERS[0] if v < 1.25 else (TIERS[1] if v < 3.05 else TIERS[2])
...
class Painter19(Painter17):
    ...
    def _floret_r(self, R, fs=1.0):
        px = self.px
        return float(np.clip(0.3 * R * fs, 1.8 * px, 9.0 * px))
    ...
    def _cluster_stamps(self, c, v, trans, rng):
        ...
        for (hx, hy, hr) in heads:
            ...
            st.append((hx - Ls[0] * hr * 0.12, hy - Ls[1] * hr * 0.12 + 0.12 * hr, hr * 0.7, rng.uniform(0, 6.3),
                       float(rng.integers(5, 8)), 0.25, 0.8 * px, cc[0], cc[1], cc[2], 0.0, 0.0, 9.0,
                       0.0, 0.0, 0.0, 1.0))
            ...
            for (s_, fx, fy, rn) in fl:
                ft = vq + 0.45 * s_ + 0.3 * rn * max(s_, 0.0) + rng.normal(0, 0.15)
                # sun-exposed clump tips: near-white cream
                ...
                st.append((fx, fy, fr * rng.uniform(0.75, 1.3), rng.uniform(0, 6.3), 5.0, 0.36, 0.85 * px,
    ...
    def _paint17(self, tree, rng, margin=10):
        ...
        for i, c in enumerate(cl):
            ...
            vv = 2.7 - 1.5 * tv + 0.75 * tl - 1.6 * max(tv - 0.1, 0.0) + 0.9 * (m['hg'] - 0.5) + m['base']
            v[i] = vv
            c['mv'] = (m['cx'], m['cy'], m['ry'])
        v = v - 1.0 * np.clip(Dsun - 0.6, 0, 1)
        # round 19: lower half of the whole crown sinks into the violet shade core
        ymid = 0.5 * (ytop + ybot)
        v = v - 0.7 * np.clip((Y - ymid) / max(ybot - ymid, 1.0), 0, 1)
        exs = np.clip((0.5 - Dsun) / 0.35, 0, 1)
        v = v + 0.9 * exs
        for i, c in enumerate(cl):
            c['ex'] = float(exs[i])
            # dab size: big in the body of a mass, fine at the crown rim
            c['fs'] = float(np.clip(math.exp(mr.normal(0.0, 0.28)) * (1.45 - 0.6 * min(rr[i], 1.2)), 0.55, 1.9))
        ...
        for j in order_m:
            ...
            vb = 0.5 - 1.2 * tv + 0.4 * tl - 1.2 * np.clip(tv - 0.1, 0, None) + 0.6 * (m['hg'] - 0.5) - 2.0 * pk
            vb = vb - 0.7 * np.clip((yy - ymid) / max(ybot - ymid, 1.0), 0, 1)
            vb = np.clip(vb, 0, 1.4)
            vb = np.round(vb * 2) / 2
        ...
        # round 19: sun-side rim glow - a 2-4 px near-white cream rim on every sun-facing blossom edge
        sh = 3.4 * px
        M = np.float32([[1, 0, -Ls[0] * sh], [0, 1, -Ls[1] * sh]])
        As = cv2.warpAffine(A, M, (W, H), borderMode=cv2.BORDER_CONSTANT)
        rim = np.clip(A - As, 0, 1) * (1 - wcov)
        rim = np.clip(rim * 1.3, 0, 1) * np.clip(0.6 + 0.8 * (1 - yyr / max(H, 1)), 0.6, 1.2)
        C += (np.array([1.16, 1.05, 0.92], np.float32) - C) * np.clip(rim * 0.85, 0, 0.92)[..., None]
        sh2 = 7.0 * px
        ...
        if not self.far or True:
            ...
            g = np.clip(Ab - A, 0, 1) * (0.34 if not self.far else 0.16)
