...
RAMP = [(0.00, '#23255a'), (0.12, '#2c3268'), (0.24, '#3a3f7a'), (0.34, '#3a5a74'), (0.44, '#3f6e5e'),
        (0.54, '#5f7f3e'), (0.66, '#95a236'), (0.78, '#d2ac3e'), (0.89, '#f4be5c'), (1.00, '#ffdca0')]
SHD_LOW = cc('#2e2c68')        # violet toward the underside of the crown
SHD_SKY = cc('#4a5294')        # sky-lit violet-blue on the upper shade flanks
RIM = np.array([1.75, 1.08, 0.55], np.float32)
RIM_HEAD = np.array([1.45, 0.78, 0.36], np.float32)    # orange-gold rim on each clump's sun-facing edge
...
def _term(shape, mx0, my0, hx, hy, rx, ry, d, Lx, Ly, rng, S4=4):
    """terminator body: a cauliflower blob like the head, shifted AWAY from the sun by d, with its own
    fresh bumps on its sun-facing arc -> head minus this = a lit plane whose inner border is a new scalloped
    line (not a copy of the outline, so the planes never read as stacked offset strokes)."""
    hh, ww = shape
    m = np.zeros((hh, ww), np.uint8)
    cx, cy = hx - Lx * d - mx0, hy - Ly * d - my0
    cv2.ellipse(m, (int(cx * S4), int(cy * S4)), (int(rx * 1.02 * S4), int(ry * 1.02 * S4)), 0, 0, 360, 1, -1,
                cv2.LINE_8, 2)
    a0 = math.atan2(Ly, Lx)
    mr = min(rx, ry)
    nb = int(rng.integers(5, 9))
    for a in a0 + rng.uniform(-1.5, 1.5, nb):
        br = mr * rng.uniform(0.1, 0.3)
        dd = 1.0 + br / max(mr, 1e-3) * rng.uniform(-0.3, 0.5)
        cv2.circle(m, (int((cx + math.cos(a) * rx * dd) * S4), int((cy + math.sin(a) * ry * dd) * S4)),
                   int(br * S4), 1, -1, cv2.LINE_8, 2)
    # a few bites (the plane pushes into the terminator body in places)
    for a in a0 + rng.uniform(-1.2, 1.2, 0):
        br = mr * rng.uniform(0.08, 0.18)
        cv2.circle(m, (int((cx + math.cos(a) * rx * 0.97) * S4), int((cy + math.sin(a) * ry * 0.97) * S4)),
                   int(br * S4), 0, -1, cv2.LINE_8, 2)
    return m


def canopy7(cv, clumps, rng, unit=1.0, clip=None, L=(0.98, -0.08), ss=2, far_haze=0.2, branches=(), wcv=None,
            lit_bias=0.0):
    ...
    RB = np.zeros((hs, ws), np.float32)          # per-head sun-facing rim (later heads overwrite earlier)
    ...
    for ci, (x, y, r, f, clipped) in enumerate(clumps):
        ...
        # sun-side heads last (in front): every lit crescent sits whole against the darker head behind
        heads.sort(key=lambda hd: 0.6 * (hd[1] + 0.3 * hd[3]) + 0.8 * ((hd[0] - cx) * Lx + (hd[1] - cy) * Ly))
        for (hx, hy, rx, ry) in heads:
            ...
            mr = min(rx, ry)
            bumps = []
            for (nb, s0, s1, d0, d1, a0_, a1_) in (
                    (int(rng.integers(7, 11)), 0.17, 0.32, 0.3, 0.7, -math.pi * 1.12, 0.45),
                    (int(rng.integers(10, 16)), 0.07, 0.13, 0.05, 0.45, -math.pi * 1.12, 0.45),
                    (int(rng.integers(6, 10)), 0.08, 0.16, 0.2, 0.6, 0.3, math.pi * 0.95)):
                angs = rng.uniform(a0_, a1_, nb)
                for a in angs:
            ...
            ex = float(np.clip(0.55 + 1.0 * (0.7 * tg + 0.4 * pc - 0.3 * td + lit_bias) + rng.normal(0, 0.06),
                               0, 1))
            # three painted value planes per head (shade core / half-tone / lit), each terminator the
            # scalloped head outline itself shifted toward the sun (cauliflower terminators)
            v_base = 0.07 + 0.13 * ex + rng.uniform(-0.03, 0.03)
            v_half = v_base + 0.13 + 0.18 * ex
            v_lit = 0.4 + 0.42 * C.smoothstep(0.3, 0.85, ex)
            v_hot = 0.95
            d2 = mr * (0.42 + 0.45 * ex)
            # shade heads take their half-tone from the sky (top), sun-flank heads from the sun (side)
            ks = float(C.smoothstep(0.2, 0.55, ex))
            hLx, hLy = Lx * ks + 0.25 * (1 - ks), Ly * ks - 1.0 * (1 - ks)
            hl_ = math.hypot(hLx, hLy)
            hLx, hLy = hLx / hl_, hLy / hl_
            d2 = mr * (0.5 + 0.35 * ex)
            half_ = m & (1 - _term(m.shape, mx0, my0, hx, hy, rx, ry, d2, hLx, hLy, rng, S4))
            dl = mr * (0.1 + 0.3 * C.smoothstep(0.3, 0.9, ex))
            lit = (m & (1 - _term(m.shape, mx0, my0, hx, hy, rx, ry, dl, Lx, Ly, rng, S4))) if ex > 0.32                 else np.zeros_like(m)
            dh = mr * 0.07
            hot = m & (1 - _shift(m, Lx * dh, Ly * dh)) if ex > 0.62 else np.zeros_like(m)
            ...
            v = v_base + (0.03 + 0.05 * ex) * pl
            v = np.where(half_ > 0, v_half + 0.04 * pl, v)
            v = np.where(lit > 0, v_lit + 0.04 * pl, v)
            ...
            sl[..., 2][mm] = swc
            sl[..., 3][mm] = 1.0
            # crisp rim on the sun-facing edge of every head (even shade heads glow a little at the edge)
            dr = 1.7 * uS
            rimm = (m & (1 - _shift(m, Lx * dr, Ly * dr))).astype(np.float32)
            rsl = RB[my0:my0 + hh_, mx0:mx0 + ww_]
            rsl[mm] = rimm[mm] * float(C.smoothstep(0.3, 0.8, ex)) * (1 - 0.4 * f)
    ...
    A = B[..., 3].copy()
    k1, k2 = max(int(12 * uS), 2), max(int(110 * uS), 4)
    ...
    made, tries = 0, 0
    while made < 26 and tries < 1500 and len(ys_):
        tries += 1
        k = int(rng.integers(0, len(ys_)))
        hx_, hy_ = xs_[k], ys_[k]
        if B[hy_, hx_, 0] > 0.36 or Tg[hy_, hx_] > 0.5:
            continue
        if clipS is not None and clipS[hy_, hx_] > 0.05:
            continue
        hr = rng.uniform(3.0, 6.5) * uS
    ...
    Au = A1.copy()
    shx = np.float32([[1, 0, -Lx * 1.9 * u], [0, 1, -Ly * 1.9 * u]])
    ...
    rim = edge * facing * C.smoothstep(0.2, 0.55, V)
    col += (RIM - col) * np.clip(0.8 * cv2.GaussianBlur(rim, (0, 0), 0.7 * u), 0, 1)[..., None]
    # ---- crisp orange-gold rim on the sun-facing edge of every clump head (inside the crown too)
    rb = cv2.resize(RB, (w, h), interpolation=cv2.INTER_AREA)
    rb = np.clip(cv2.GaussianBlur(rb, (0, 0), 0.45 * u) * 1.15, 0, 1)
    col += (RIM_HEAD - col) * (0.8 * rb)[..., None]
    # ---- lost edges on the down / away-from-sun silhouette (soft alpha, in shade)
    softA = cv2.GaussianBlur(A1, (0, 0), 3.0 * u)
    away = C.smoothstep(0.35, 0.85, (gx1 * Lx + gy1 * Ly) / gm)
    dn = np.maximum(C.smoothstep(0.45, 0.9, gy1 / gm), 0.85 * away) * C.smoothstep(0.42, 0.2, V)
