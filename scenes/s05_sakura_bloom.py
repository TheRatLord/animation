"""s05_sakura round 4: blossom masses painted as layered flower CLUMPS (replaces the smooth banded masses).

Each big mass = its lacy silhouette split into 6-11 clumps of strongly varied size, painted top (behind) to bottom
(in front).  Value = mass-level light (lit top / lavender underside) + each clump's own cap light, cut into crisp
painted bands; a thin contact shade where a front clump overlaps the one behind; lit clump tops broken by
5-petal floret clusters; clustered floret texture in the lit top third, faint darker florets in the shade.
Sky pinholes are irregular chains of small negative shapes near the edges whose rims are re-covered by florets
(jagged, no outline).  Limbs are truncated where they first enter a mass (no wood strokes beyond it), limbs that
never reach a mass are dropped, floating hem clumps are removed.
"""
import math
import numpy as np
import cv2
...
class Painter(T2.Painter):
    def __init__(self, k, clumps=(16, 26), **kw):
        super().__init__(k, **kw)
        self.nclumps = clumps
    ...
    def mass(self, card, m, to_px, rng, kind='mass', occl=None, cutmap=None):
        ...
        cand = np.argwhere(sd < -0.08 * R)
        if len(cand):
            cand = cand[mr.permutation(len(cand))[:600]]
            for (py, px) in cand:
                # strongly varied sizes: a few big heads, several mid, some small
                u_ = mr.random()
                rc = R * (0.34 if u_ < 0.2 else (0.24 if u_ < 0.55 else 0.15)) * mr.uniform(0.85, 1.15)
                if not big:
                    rc = R * mr.uniform(0.45, 0.65)
                qx, qy = px + 0.5 + X0 - cx, py + 0.5 + Y0 - cy
                if all(math.hypot(qx - c[0], qy - c[1]) > 0.55 * (rc + c[2]) for c in clumps):
        ...
        for j, (qx, qy, rc) in enumerate(clumps):
            ...
            cs[band] = np.maximum(cs[band], ((1 - sdc / wsh) * (0.3 + 0.7 * away))[band])
            cs[inj] = 0
            ob[inj] = j
            ...
            vloc = np.clip(-0.28 + 1.0 * capm - 0.25 * np.clip(ny_, 0, 1), -1, 1)
        ...
        col = col * (1 - 0.6 * tr[..., None]) + pal['trans'] * (0.6 * tr)[..., None]
        ...
        # ---- sky pinholes: irregular chains of small negative shapes near the thin edges
        hole = np.zeros((h, w), np.float32)
        if big and fr >= 1.0 and self.holes > 0:
            nh = int(round(self.holes * mr.integers(4, 9) * min(1.0, R / 60.0)))
            cand = np.argwhere((sd < -0.4 * bw) & (sd > -2.4 * bw - 0.05 * R) & (hf < 0.8))
            if nh > 0 and len(cand):
                pick = cand[mr.choice(len(cand), min(nh, len(cand)), replace=False)]
                for (py, px) in pick:
                    pts = []
                    x_, y_ = float(px), float(py)
                    for _ in range(int(mr.integers(3, 7))):
                        rr_ = fr * mr.uniform(0.6, 1.5)
                        pts.append((x_, y_, rr_))
                        a_ = mr.uniform(0, 6.283)
                        x_ += math.cos(a_) * fr * 1.3
                        y_ += math.sin(a_) * fr * 1.3
                    ext = int(4 * fr + 4)
                    ...
                    # rim re-covered by single florets (jagged 5-petal edge, no outline)
                    for (hx, hy, rr_) in pts:
                        for _ in range(int(mr.integers(1, 3))):
                            a_ = mr.uniform(0, 6.283)
                            d_ = rr_ + fr * mr.uniform(0.1, 0.6)
        ...
        if self.detail > 0 and fr >= 2.0 and big:
            ...
            p = np.maximum(p, 0.85 * (capf[iy, ix] > 0.5) * self.detail)
            ok = (sd[iy, ix] < -bl_mean(bw_l)) & (mr.random(ix.shape) < p)
            for px, py, v_ in zip(jx[ok], jy[ok], vv[ok]):
                tone = 1.04 if v_ > 0.6 else (1.1 if v_ < 0.19 else 1.06)
                for _ in range(int(mr.integers(3, 7))):
        ...
        sh = _shift(a, Lx * rimw * 1.5, Ly * rimw * 1.5)
    ...
    def paint(self, tree, rng, ss_margin=4):
        ...
        yb = oy - cb * k
        shade = 0.85 * _ss(yb + 0.3 * k, yb - 0.6 * k, ys_) * np.ones((1, W), np.float32)
