"""Round-13 cloud layout for s07_comet_night: broken banks + diagonal streamers clustered like yn_05."""
...
def clusters(Wp, W, H, my):
    ...
    def add(fx, fy, L, T, ang, kind, amt=1.0, warm=0.0, dens=1.0):
        x, y = fp(fx, fy)
        out.append(dict(x=x, y=y, L=L * W, T=T * H, ang=ang, kind=kind, amt=amt, warm=warm, dens=dens))
    A = 27.0   # streamers follow the comet's direction
    # ---- upper-left broken bank: torn streamers + a couple of puffy heads
    add(0.08, 0.1, 0.22, 0.05, A, 'st', 0.95)
    add(0.2, 0.19, 0.2, 0.06, A - 4, 'st', 1.0)
    add(0.14, 0.16, 0.12, 0.11, 10, 'cu', 1.0)
