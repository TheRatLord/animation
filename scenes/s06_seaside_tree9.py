...
PLANES = ['#172335', '#1c2e42', '#233c4c', '#2d4d56', '#4a6448', '#7d7a3e', '#b88a40', '#eaa654', '#ffcc88']
STEPS = [0.14, 0.26, 0.37, 0.47, 0.57, 0.67, 0.77, 0.88]
SHD_LOW = cc('#1a2644')        # violet core toward the underside
SKY_LIT = cc('#44597f')        # sky-lit blue-violet on the upper shade tops
EX_FLOOR = 0.22
N_HOLES = 6
GLOW = 0.9
# interior limbs (frame fractions of the nominal frame: x, y of the base, heading, width px@1080/2, segments)
LIMBS = ((0.088, 0.55, -1.35, 22.0, 7), (0.13, 0.54, -1.05, 16.0, 6), (0.165, 0.52, -1.9, 12.0, 6))
FLAT_BIG, FLAT_MED, EDGE_BAND = 18.0, 6.0, 16.0
RIM = np.array([1.5, 0.98, 0.52], np.float32)
BRANCH, BRANCH_LIT = cc('#141a2a'), cc('#8a5a3a')
...
def canopy9(cv, clumps, rng, unit=1.0, clip=None, L=(0.88, -0.47), ss=2, far_haze=0.2, branches=(), wcv=None, anchor=None, holes=None, expo=1.0, ex_floor=None,
            **_):
    ...
    def exposure(px, py, pc):
        ...
        return float(np.clip(0.42 + 0.75 * tg + 0.35 * pc - 0.3 * td, 0, 1)) * expo, iy, ix
    ...
    for ci, (x, y, r, f, clipped) in enumerate(clumps):
        ...
        for (hx, hy, hr) in crowns:
            ...
            exf = EX_FLOOR if ex_floor is None else ex_floor
            ex = exf + (1 - exf) * ex      # every big head keeps its own lit top-right plane
    ...
    # broad flat value masses inside the shade: the per-lobe bead shading is replaced by an alpha-normalised
    # blur of the value (big clump forms survive, the same-size dabs do not); per-lobe detail is kept only on
    # the silhouette band and on the lit planes
    Asafe = np.maximum(A, 0.0)
    Vs = cv2.GaussianBlur(V * Asafe, (0, 0), FLAT_BIG * uS) / np.maximum(cv2.GaussianBlur(Asafe, (0, 0), FLAT_BIG * uS), 1e-4)
    Vm = cv2.GaussianBlur(V * Asafe, (0, 0), FLAT_MED * uS) / np.maximum(cv2.GaussianBlur(Asafe, (0, 0), FLAT_MED * uS), 1e-4)
    Dd = cv2.distanceTransform((A > 0.5).astype(np.uint8), cv2.DIST_L2, 5)
    DW = np.maximum(C.smoothstep(EDGE_BAND * uS, EDGE_BAND * 0.35 * uS, Dd), C.smoothstep(0.34, 0.5, Vm))
    Vb = Vs * 0.5 + Vm * 0.5
    V = Vb + (V - Vb) * DW
    Vp = V + (lf - 0.55) * (0.012 + 0.09 * np.exp(-((V - 0.52) / 0.16) ** 2)) * (0.3 + 0.7 * DW)
    ...
    # ---- interior structure: dark recesses with the trunk / limbs reading through them, and a few big sky
    # holes through the canopy (a twig crossing some of them)
    r2 = np.random.default_rng(907)
    lfc = C.smoothstep(0.5, 0.72, _leaf_field(ws, hs, 15.0 * uS, 813))        # leaf clusters in front
    if anchor is not None:
        aox, aoy, aW, aH = anchor

        def cp(fx, fy):
            return ((aox + fx * aW - X0) * S, (aoy + fy * aH - Y0) * S)
        BM = np.zeros((hs, ws), np.uint8)
        RB = np.zeros((hs, ws), np.uint8)
        k4 = 4
        for (fx, fy, ang0, wd0, nseg) in LIMBS:
            ...
            for k in range(nseg):
                ...
                if k in (2, 4) and r2.random() < 0.8:          # a side limb
                    a2 = ang + r2.choice([-1, 1]) * r2.uniform(0.6, 1.0)
                    q = [pts[-1]]
                    for _k in range(3):
                        q.append((q[-1][0] + math.cos(a2) * 14 * uS, q[-1][1] + math.sin(a2) * 14 * uS, q[-1][2] * 0.7))
                        a2 += r2.normal(0, 0.3)
                    for i in range(len(q) - 1):
                        cv2.line(BM, (int(q[i][0] * k4), int(q[i][1] * k4)), (int(q[i + 1][0] * k4), int(q[i + 1][1] * k4)),
                                 255, max(int(round(q[i][2] * 0.6)), 1), cv2.LINE_AA, 2)
                        cv2.line(RB, (int(q[i][0] * k4), int(q[i][1] * k4)), (int(q[i + 1][0] * k4), int(q[i + 1][1] * k4)),
                                 255, max(int(round(q[i][2] * 3.5)), 2), cv2.LINE_AA, 2)
            for i in range(len(pts) - 1):
                cv2.line(BM, (int(pts[i][0] * k4), int(pts[i][1] * k4)), (int(pts[i + 1][0] * k4), int(pts[i + 1][1] * k4)),
                         255, max(int(round(pts[i][2])), 1), cv2.LINE_AA, 2)
                cv2.line(RB, (int(pts[i][0] * k4), int(pts[i][1] * k4)), (int(pts[i + 1][0] * k4), int(pts[i + 1][1] * k4)),
                         255, max(int(round(pts[i][2] * 3.2)), 2), cv2.LINE_AA, 2)
        bm = BM.astype(np.float32) / 255
        rb = cv2.GaussianBlur(RB.astype(np.float32) / 255, (0, 0), 4 * uS)
        shade_ = C.smoothstep(0.55, 0.35, V) * A
        rec = np.clip(rb * 1.4, 0, 1) * (1 - 0.35 * lfc) * shade_
        col += (cc('#101a28') - col) * (0.75 * rec)[..., None]
        vis = bm * (1 - C.smoothstep(0.84, 0.95, _leaf_field(ws, hs, 24.0 * uS, 814))) * C.smoothstep(0.62, 0.45, V) * A
        shx2 = np.float32([[1, 0, -Lx * 1.5 * uS], [0, 1, -Ly * 1.5 * uS]])
        bedge = np.clip(bm - cv2.warpAffine(bm, shx2, (ws, hs)), 0, 1)
        bc = BRANCH[None, None] + (cc('#6a4a3c') - BRANCH)[None, None] * (0.9 * bedge)[..., None]
        bl = np.clip(bm - cv2.warpAffine(bm, np.float32([[1, 0, 2.0 * uS], [0, 1, 0]]), (ws, hs)), 0, 1)
        bc += (cc('#34405e') - bc) * (0.6 * bl)[..., None]          # sky-lit bark on the shade side
        col += (bc - col) * vis[..., None]
    # big sky holes: interior shade areas, only where sky (not the hill) is behind the crown
    Dh = cv2.distanceTransform((A > 0.5).astype(np.uint8), cv2.DIST_L2, 5)
    okh = (Dh > 14 * uS) & (Dh < 90 * uS) & (V < 0.44)
    if clipS is not None:
        okh &= clipS < 0.02
    yh, xh = np.nonzero(okh)
    holes_n = holes
    holes = []
    tries = 0
    while len(holes) < (N_HOLES if holes_n is None else holes_n) and tries < 3000 and len(yh):
        tries += 1
        k = int(r2.integers(0, len(yh)))
        hx_, hy_ = float(xh[k]), float(yh[k])
        if any(math.hypot(hx_ - a_, hy_ - b_) < 70 * uS for a_, b_ in holes):
            continue
        holes.append((hx_, hy_))
        hr = r2.uniform(10.0, 18.0) * uS
        for _k in range(int(r2.integers(3, 6))):
            cx_, cy_ = hx_ + r2.normal(0, hr * 0.6), hy_ + r2.normal(0, hr * 0.45)
            rr_ = hr * r2.uniform(0.45, 0.9)
            x0 = int(max(cx_ - rr_ * 1.4 - 3, 0)); x1 = int(min(cx_ + rr_ * 1.4 + 4, ws))
            y0 = int(max(cy_ - rr_ * 1.4 - 3, 0)); y1 = int(min(cy_ + rr_ * 1.4 + 4, hs))
            if x1 <= x0 or y1 <= y0:
                continue
            yy, xx = np.mgrid[y0:y1, x0:x1].astype(np.float32)
            sd, _a, _b, _c = _lobe_geom(xx, yy, cx_, cy_, rr_, 4.0 * uS, r2.uniform(0, 6), 1.2 * uS, 0.3, r2.uniform(0, 6))
            HM[y0:y1, x0:x1] = np.maximum(HM[y0:y1, x0:x1], np.clip(sd / 1.1 + 0.5, 0, 1))
        # a twig crossing the hole (on the branch layer behind the leaves)
        if r2.random() < 0.75:
            a_ = r2.uniform(-2.4, -0.7)
            ln_ = hr * 2.2
            m = np.zeros((hs, ws), np.uint8)
            cv2.line(m, (int((hx_ - math.cos(a_) * ln_) * 4), int((hy_ - math.sin(a_) * ln_) * 4)),
                     (int((hx_ + math.cos(a_) * ln_) * 4), int((hy_ + math.sin(a_) * ln_) * 4)), 255,
                     max(int(round(1.6 * uS)), 1), cv2.LINE_AA, 2)
            mf = m.astype(np.float32) / 255 * C.smoothstep(0.6, 0.95, A)
            BR[:] = BR * (1 - mf[..., None]) + BRANCH[None, None] * mf[..., None]
            BA[:] = np.maximum(BA, mf)
    # hole rims: the leaves round a hole catch a little sky light (lavender bounce), crisp
    hr_ring = np.clip(cv2.dilate(HM, np.ones((int(3 * uS) * 2 + 1,) * 2, np.uint8)) - HM, 0, 1)
    col += (cc('#5e6a9a') - col) * (0.35 * hr_ring * (1 - lfc * 0.5))[..., None]
    A2 = A2 * (1 - HM)
    ...
    # slight bloom where the sunlit rim meets the sky: a warm low-alpha halo just outside the silhouette
    if GLOW > 0:
        rs = cv2.resize(np.ascontiguousarray(rim * A), (w, h), interpolation=cv2.INTER_AREA)
        g = cv2.GaussianBlur(rs, (0, 0), 5 * u) * 0.6 + cv2.GaussianBlur(rs, (0, 0), 14 * u) * 0.8
