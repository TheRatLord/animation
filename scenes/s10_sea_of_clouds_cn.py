"""s10 cumulus towers: copy of the s01 painted-lobe tower renderer (palette / light passed in by s10).
...
def render_tower(T, pw, ph, sun, W, ss=2, L=(0.72, -0.68, 0.06), P=PAL, sky=None, seed=1, thr0=0.3, thr_k=0.36, crown_v=None, lobe_w=0.42):
    ...
    wlev = lobe_w * (0.45 + 0.55 * _ss(0.08, 0.4, vpix))       # the shadowed foot is less articulated
    for j in (2, 1, 0):
        m = (lev > j)
        if not m.any():
            continue
    ...
    thr = thr0 - thr_k * np.clip(v, 0, 1)
    le = ndl + 0.5 * (ndl_f - 0.15) - thr - 0.08 * sh - 0.05 * crev
    del n_f_keep
    ...
    if crown_v is not None:
        # s10: only the crown catches the low sun; the lower body stays ONE cool shadow mass
        # (no isolated lit caps on lobes inside the shadow)
        cmask = _ss(crown_v[0], crown_v[1], v + 0.12 * ndl_f + 0.12 * ndl)
        lit = lit * cmask
    else:
        cmask = 1.0
    capk = _ss(0.14, 0.22, le) * cmask                  # brightest cap
    hotk = _ss(0.34, 0.39, le) * _ss(0.45, 0.8, v) * cmask
    half = _ss(-0.15, -0.11, le) * (1 - lit) * (0.15 + 0.85 * cmask)   # half-tone just below the terminator
