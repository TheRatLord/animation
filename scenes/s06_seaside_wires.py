...
def draw(img, wires, pan, K, ox, oy, f, u, color=(0.17, 0.155, 0.28), sheen=(1.7, 1.12, 0.66)):
    H, W = img.shape[:2]
    segs = []
    ylo, yhi = H, 0
    for (px, py, zz, kk) in wires:
        ...
        wd = np.maximum(f * 0.02 / zz_, 0.6 * u)
    ...
    reg *= (1 - m)[..., None]
    reg += c * m[..., None]
    reg += (np.asarray(sheen, np.float32) - c) * (ml * 0.85)[..., None]
    return img
